...
There are some sub-commands for using `py-spy`.
...
``` bash
py-spy record --pid 12345
# OR
py-spy record -- python myprogram.py
```
That sub-command will generate a file containing raw data representing stack traces.

### Top
It will generate a [top-like](https://linux.die.net/man/1/top) live view of your python program:
...
```bash
py-spy display -g RAW_DATA_FILE -s START_TIME_OFFSET -e END_TIME_OFFSET
```
...
## Sub-interval Analysis Support

Py-spy is capable of filtering existing results to generate sub-interval results within the time period of preivous results. It is convenient for users
to have a detailed obversation on some specific intervals with the large time interval when they spot some performance issues.

To use this feature, users should first profile a large time period and after that they can use the following command.

```bash
py-spy display -g RAW_DATA_FILE -s START_TIME_OFFSET -e END_TIME_OFFSET
```

Users need to specify the raw data file which is generated by the `record` sub-command and two time stamps offset to the starting point of previous profiling
to represent the target interval. Then a flame graph named `${RAW_DATA_FILE}.svg` will be generated.

## Black List Support

Sometimes, users does not want some functions to show in the results e.g. some functions make the thread idle. They can specify a black list and let the profiler
load that list and filter the result. Users can use `-i` option in the command to specify the black list file. Within that file, users can use following line format
```bash
function_name program_name(module_name) check_mode
```
Note that the `function_name` and `program_name(module_name)` are informantion in the deepest stack frame. The check mode includes that

* X: Exact the same program
* E: End with the program_name
* C: Contain the program_name
